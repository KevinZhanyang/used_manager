  ...
  <div>
    <Card>
      <p slot="title">
       ...
       <Row>
        <Input v-model="param.name" placeholder="请输入名称搜索..." style="width: 200px" />
    ...
    list () {
      ...
      album.query(qData, {
        success: function (data) {
          ...
          _this.itemList = data['albumList']
        }
      })
    },
    view (p) {
      this.$router.push({path: '/album/view', name: 'albumDetail', params: {recId: p.row.recId}})
    },
    add () {
      this.$router.push({path: '/album/new', name: 'albumNew', params: {}})
    },
    edit (p) {
      this.$router.push({path: '/album/new', name: 'albumNew', params: {recId: p.row.recId}})
    },
    deleteBy (p) {
      var _this = this
      var id = p.row.recId
